...
function AdminDashboard() {
  ...
  return (
    <div className="p-6 bg-gray-900 text-white min-h-screen">
      ...
      <div className="bg-gray-800 p-4 rounded shadow-lg">
        <h2 className="text-lg font-semibold text-gray-400 mb-4">Payment Details</h2>
        {loading ? (
            ...
            <table className="min-w-full table-auto">
              <thead>
                <tr className="bg-gray-700">
                  ...
                  <th className="px-4 py-2 text-left text-gray-300">Payment ID</th>
              ...
              <tbody>
                {payments?.map((payment) => (
                  <tr key={payment._id} className="border-b border-gray-700">
                    <td className="px-4 py-2 text-gray-200">{payment.user?.name}</td>
                    <td className="px-4 py-2 text-gray-200">{payment.razorpay_payment_id}</td>
                    <td className="px-4 py-2 text-gray-200">{payment.razorpay_order_id}</td>
                    <td className="px-4 py-2 text-gray-200">{payment.razorpay_signature}</td>
                    <td className="px-4 py-2 text-gray-200">{new Date(payment.createdAt).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
